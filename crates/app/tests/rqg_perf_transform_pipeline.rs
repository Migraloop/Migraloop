...
//!
//! Fixed Transform Pipeline microbench on the contract/stub path
//! (Affect Analysis → Derived → Delivery; issue #231 / ADR-0029):
...
/// Committed baseline shape under `ci/rqg/transform_pipeline_microbench_baseline.json`.
...
fn baseline_path() -> PathBuf {
    ...
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../ci/rqg/transform_pipeline_microbench_baseline.json")
}
...
fn deployment_transform_customers(mongo_database: &str) -> String {
    format!(
        r#"
apiVersion: migraloop.dev/v1
kind: Deployment
metadata:
  name: rqg-perf-transform
spec:
  source:
    kind: oracle
    host: stub
...
async fn run_timed_microbench(seed_rows: u64) -> PerfMeasurement {
    ...
    let config = write_config(
        &dir,
        "deployment.yaml",
        &deployment_transform_customers(&mongo_database),
    );
    ...
    assert!(
        apply_out.contains("Derived Dataset materialized")
            || apply_out.to_ascii_lowercase().contains("derived"),
        "expected Derived materialization on Transform IL, got:\n{apply_out}"
    );
    assert!(
    ...
    assert!(
        sync_out.to_ascii_lowercase().contains("affect")
            || sync_out.contains("Delivery complete")
            || sync_out.to_ascii_lowercase().contains("deliver"),
        "expected Affect Analysis / Delivery after Incremental Capture, got:\n{sync_out}"
    );
    ...
    println!(
        "rqg-perf transform-pipeline microbench seed_rows={seed_rows} duration_ms={duration_ms} rows_per_s={rows_per_s:.3} rows_applied={seed_rows}"
    );
...
async fn transform_pipeline_microbench_meets_committed_baseline() {
