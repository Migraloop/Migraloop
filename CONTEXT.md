...
**Scenario Namespace**:
The Lab Scenario’s isolated slice of Lab Source/Target (and related Pipeline names)—its own schema/tables, collections, and identifiers—so leftover data from one Scenario does not collide with another (for example on primary keys). Re-running the same Scenario **fully removes** that Scenario Namespace before recreating and running it.
_Avoid_: Sharing one global table/PK space across Scenarios, soft overwrite without remove, treating customer DB schemas as a Scenario Namespace
