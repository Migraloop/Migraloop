...
_Avoid_: Zero-impact backfill, unbounded full-table slam, assuming a separate replica connection is required for correctness, gap-tolerant cutover, relying on later repair as the primary cutover strategy
...
_Avoid_: Raw table (ambiguous), source mirror, target table, per-pipeline copy of the same source table, whole-schema capture by default
