...
**Lab Scenario**:
A predefined, manually invoked recipe that exercises a slice of real product behavior inside the Local Sync Lab (for example bulk Source inserts, or a multi-table Transform Pipeline). Each Scenario is **self-contained**: it prepares the Source schema, Deployment/Pipelines, data actions, and checks it needs, so the operator can choose Scenarios without sharing one fat Fixture. Before (or as) it runs, it must clear prior Scenario-owned state that would conflict (for example leftover rows that collide on primary keys)—Scenarios must not step on each other. A Scenario run reports **correctness and operational metrics with equal weight**: expected Target/platform outcomes (pass/fail) plus lag, throughput, duration, and any Scenario-defined thresholds that can also fail the run. That reporting does not replace live DB access. The catalog aims to cover first-class capabilities; it is not the Release Quality Gate and does not replace automated CI.
_Avoid_: Unit test, CI job, contract/stub harness case, ad-hoc SQL alone as the Scenario concept, Scenario-only Lab with no direct DB inspection, correctness-only Scenario results with metrics as optional decoration, shared mutable Fixture that lets Scenarios collide on keys or Pipelines
