...
**Base Dataset**:
A platform-managed copy of a source table or collection, kept aligned by Sync, close to the source shape. It is the unit Rich Transforms and direct Pipelines may read; it is not the user's source or target database. Within a Deployment, each source table/collection has at most one Base Dataset, shared by every Pipeline that needs it—never captured or stored once per Pipeline. Sync captures **only tables/columns referenced by Pipelines** in that Deployment—not an entire schema or database mirror (see ADR-0019). When a new Pipeline needs a table with no Base Dataset yet, run **table-level Initial Load for that table only**; existing Bases stay on Incremental Capture and are not reloaded.
_Avoid_: Raw table (ambiguous), source mirror, target table, per-pipeline copy of the same source table, whole-schema capture by default, full Deployment reload when one new table is needed
