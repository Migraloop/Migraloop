...
**Lab Scenario**:
A predefined, manually invoked recipe that exercises a slice of real product behavior inside the Local Sync Lab (for example bulk Source inserts, or a multi-table Transform Pipeline). The operator runs it; the Scenario performs its own setup/actions rather than requiring the operator to script the flow. A Scenario run reports its own outcomes—checks, metrics, and effects—so the catalog can show coverage of first-class capabilities; that reporting does not replace live DB access. It is not the Release Quality Gate and does not replace automated CI.
_Avoid_: Unit test, CI job, contract/stub harness case, ad-hoc SQL alone as the Scenario concept, Scenario-only Lab with no direct DB inspection
