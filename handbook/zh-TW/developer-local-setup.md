...
1. **實作 engine interface**
   ...
   - 預設 Operator CLI `apply`／`run`／`sync` 仍經由上述 factories 建構 v1 Oracle LogMiner 與 MongoDB adapters。完整 Incremental Sync 在 runtime interface 接受 typed `SyncOptions`（`run_incremental_sync`／`run_incremental_sync_with_engines`／`sync_incremental_with_options`）。RQG contract twins 與 Lab fault paths 以 typed SyncOptions（隱藏的 `migraloop sync` flags 或顯式 structs）傳入 Poison／delay／fail-after／queue-capacity knobs，並以 typed ApplyOptions（隱藏的 `migraloop apply` flags 或顯式 structs／`apply_with_options`）傳入 Initial Load chunk／rate／pause／store-delay knobs——不以 process env 作為主要 adapter。Continuous Sync idle poll 間隔 typed 於 SyncOptions（`poll_interval_ms`；`run` 上的 hidden `--sync-poll-interval-ms`）。Fake adapters 可走 production Sync path 且不依賴 Oracle-kind string gates。Poison quarantine、Schema Change pause、與 bounded Backpressure 在 Incremental Sync 內仍是 distinct policies。既有 fault-injection env knobs（`MIGRALOOP_DELIVERY_POISON_IDENTITIES`、`MIGRALOOP_DELIVERY_DELAY_MS` 等）僅在未設定 typed overrides 時保留為薄的暫時 compat shim（`SyncOptions::for_cli`／`from_env_compat`）。Legacy Lab-inject env（如 `MIGRALOOP_INITIAL_LOAD_PAUSE_AFTER_CHUNKS`／`MIGRALOOP_INITIAL_LOAD_STORE_DELAY_MS`）在 typed overrides 未設定時仍為薄 compat shim。Operator knobs 如 `MIGRALOOP_SYNC_QUEUE_CAPACITY`／`MIGRALOOP_POISON_MAX_ATTEMPTS`／`MIGRALOOP_INITIAL_LOAD_CHUNK_SIZE`／`MIGRALOOP_INITIAL_LOAD_ROWS_PER_SEC`／`MIGRALOOP_SYNC_POLL_INTERVAL_MS` 仍可經由 env 生效。
