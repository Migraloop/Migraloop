...
# 本機 cleartext 即可。正式環境 store TLS：加上 ?sslmode=require&sslrootcert=/path/to/ca.pem
...
2. 註冊 Scenario id，並為 `workload.product_path`（catalog Scenarios 必填）只實作 rare escapes 的 thin hooks。共用 Namespace wipe／prepare（以及可選 `mutate_sql`）來自 `namespace.lifecycle`；isomorphic Managed／Derived／Target correctness 來自 executable `checks.correctness`——不要再複製另一份 prepare／remove 三元組或 fat inspect assert。Recipe 的 `workload`／`namespace.lifecycle`／executable `checks.correctness`／`thresholds` 是 recipe-driven runner 介面——不要把 threshold 數值、Namespace wipe／prepare SQL、isomorphic Managed／Derived／Target inspect assert，或共用 product-path 序列再複製成 Rust constants。
...
新的 Source System 或 Target System kind 應接在穩定介面上。**不要**為了新 engine 重塑 Sync、Rich Transform、Delivery、Deployment runtime 或 Platform Store 概念——實作 seam、補齊 Operator prerequisites 文件，再完成 capability ladder（ADR-0024 / ADR-0025 / ADR-0028）。
...
1. **實作 engine interface**
   ...
   - Source discovery 的 columns 在 `SourceEngine` seam 暴露 engine-agnostic `data_type`／shared `ColumnShape`。Oracle allow-list、size caps、與 type-brand 命名留在 adapter-private（ADR-0018）。NUMBER→Mongo classification 只放在 `migraloop-types` 的 `ColumnShape` 旁（ADR-0023）——不要在 capture／delivery 再留 twin helpers。
   ...
   - Deployment runtime 的 public surface 僅限 Operator Deployment verbs（apply、Incremental Sync／supervise、Pipeline lifecycle、Source Alignment Check、Drift Check、status inventory、inspect）加上上述 factory entry points。不要從 `migraloop-runtime` 外部呼叫已 demote 的 internal helpers。Continuous Sync／supervise 偏好在 Operator edge 開啟的 Platform Store session。
   - 預設 Operator CLI `apply`／`run`／`sync` 仍經由上述 factories 建構 v1 Oracle LogMiner 與 MongoDB adapters。kind 選擇留在 factories；engine 經 factory 選出或注入後，apply／Incremental Sync orchestration 不再以 `kind == "oracle"` 重閘。完整 Incremental Sync 在 runtime interface 接受 typed `SyncOptions`（`run_incremental_sync`／`run_incremental_sync_with_engines`；continuous／supervise 保留真正的 poll 與 restart policy）。僅重新命名 OneShot verb 的 Sync entry aliases 已收斂（#208）。Apply／Initial Load 接受 typed ApplyOptions 與可選的 injected engines（`apply_with_options`／`apply_with_engines`）。RQG contract twins 與 Lab fault paths 以 typed SyncOptions（隱藏的 `migraloop sync` flags 或顯式 structs）傳入 Poison／delay／fail-after／queue-capacity knobs，並以 typed ApplyOptions（隱藏的 `migraloop apply` flags 或顯式 structs／`apply_with_options`）傳入 Initial Load chunk／rate／pause／store-delay knobs——不以 process env 作為主要 adapter。Continuous Sync idle poll 間隔 typed 於 SyncOptions（`poll_interval_ms`；`run` 上的 hidden `--sync-poll-interval-ms`）。Fake adapters 可走 production apply 與 Sync path 且不依賴 orchestration kind gates。Poison quarantine、Schema Change pause、與 bounded Backpressure 在 Incremental Sync 內仍是 distinct policies。既有 fault-injection env knobs（`MIGRALOOP_DELIVERY_POISON_IDENTITIES`、`MIGRALOOP_DELIVERY_DELAY_MS` 等）僅在未設定 typed overrides 時保留為薄的暫時 compat shim（`SyncOptions::for_cli`／`from_env_compat`）。Legacy Lab-inject env（如 `MIGRALOOP_INITIAL_LOAD_PAUSE_AFTER_CHUNKS`／`MIGRALOOP_INITIAL_LOAD_STORE_DELAY_MS`）在 typed overrides 未設定時仍為薄 compat shim。Operator knobs 如 `MIGRALOOP_SYNC_QUEUE_CAPACITY`／`MIGRALOOP_POISON_MAX_ATTEMPTS`／`MIGRALOOP_INITIAL_LOAD_CHUNK_SIZE`／`MIGRALOOP_INITIAL_LOAD_ROWS_PER_SEC`／`MIGRALOOP_SYNC_POLL_INTERVAL_MS` 仍可經由 env 生效。
...
2. **Prerequisites 與 handbook**
   ...
   - prerequisites 未滿足時在 apply/sync **fail fast**；預設不要自動改動客戶 Source/Target 設定。
3. **Lab Scenario**
   - 在 `lab/scenarios/<id>/` 新增可選擇的 Lab Scenario，於真實 product path 上演習新 engine（同上 recipe-driven runner checklist）。以 Lab-only bindings 做 Namespace isolation。
4. **CI contract twin**
   - 在 `crates/app/tests` 新增**非 ignored** 的 contract-path twin，並更新 `docs/rqg/CI_TWIN_COVERAGE.md`。優先 contract/stub + Platform Store/Mongo（或實作相同 interface 的 in-memory fake）——不要把 Lab catalog 當 CI。
5. **Packaging guards**
   - 維持 modular monorepo + 單一 `migraloop` binary（ADR-0024）。不要引入第二個 Platform Store engine（ADR-0001）。

Seam 測試可用 in-memory `FakeSource`／`FakeTarget`（含 injected apply／Initial Load 與完整 Incremental Sync）；它們不是第二個 production engine。
...
| **rqg-perf** | contract/stub 上固定 Direct + Transform Pipeline microbench，對照 committed baselines（`allowed_regression_pct` 約 55，因應 hosted runner 噪訊；各最多 3 次 attempts） | 下方 CI 對齊 env，再 `bash ci/rqg/run_direct_pipeline_microbench.sh` 與 `bash ci/rqg/run_transform_pipeline_microbench.sh` |
