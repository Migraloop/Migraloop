...
2. 註冊 Scenario id，並為 `workload.product_path`（catalog Scenarios 必填）只實作 rare escapes + correctness assert 的 thin hooks。共用 Namespace wipe／prepare（以及可選 `mutate_sql`）來自 `namespace.lifecycle`——不要再複製另一份 prepare／remove 三元組。Recipe 的 `workload`／`namespace.lifecycle`／`checks`／`thresholds` 是 recipe-driven runner 介面——不要把 threshold 數值、Namespace wipe／prepare SQL，或共用 product-path 序列再複製成 Rust constants。
