...
1. **實作 engine interface**
   ...
   - 預設 Operator CLI `apply`／`run`／`sync` 仍經由上述 factories 建構 v1 Oracle LogMiner 與 MongoDB adapters。in-process seam 測試可用 injected engines 加上 typed `SyncOptions`（`run_incremental_sync_with_engines`）跑完整 Incremental Sync，讓 Fake adapters 走 production Sync path——含 Poison／Backpressure fault knobs——且不依賴 Oracle-kind string gates 或 process env vars。Poison quarantine、Schema Change pause、與 bounded Backpressure 在 Incremental Sync 內仍是 distinct policies。既有 env knobs（`MIGRALOOP_DELIVERY_POISON_IDENTITIES`、`MIGRALOOP_SYNC_QUEUE_CAPACITY` 等）透過 `SyncOptions::from_env_compat` 保留為薄的暫時 compat shim，讓既有 RQG／Lab twins 繼續通過。
