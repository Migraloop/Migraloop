...
**Local Sync Lab** 可拋棄預設（`migraloop lab up`）刻意方便本機開發，並在 bring-up 後印出（`ORACLE_PASSWORD=lab_oracle`、`MONGO_PASSWORD=lab_mongo`、Platform Store `migraloop`/`migraloop`）。Lab Scenario 執行（`migraloop lab scenario run …`）會以相同的 Lab-only secret references，對可拋棄 Fixture 跑真實 `apply`/`sync`。僅供 Lab 使用；絕不要把 Lab 指令或 Scenario 設定指向客戶正式環境資料庫。
