...
**Local Sync Lab：** `migraloop lab up` 會佈建可拋棄的 Oracle Source，並已滿足 Lab 使用所需的 database-level prerequisites（ARCHIVELOG + database supplemental logging + sync-user grants）。當 Lab Scenario（或你）建立 Pipeline 參照的資料表時，仍須套用 table-level supplemental logging—例如 `migraloop lab scenario run direct-pipeline`、`transform-pipeline` 或 `concurrent-source-workload` 會在其 Scenario Namespace 資料表加上 `SUPPLEMENTAL LOG DATA (ALL) COLUMNS`，再走真實 `apply` / LogMiner `sync`（需要 host Instant Client / `LD_LIBRARY_PATH`）。重跑同一 Scenario 會先 drop 再重建那些 Namespace 資料表；`lab scenario remove` 可在不重跑的情況下清除。Lab 不會變更客戶／正式環境資料庫。
