...
See [Rich Transform](rich-transform.md) for operator shapes (`project`, `addFields`, `rename`, `remove`, `filter`, `equiLookup`, `groupBy` with sum/count/min/max/avg, `distinct`, `addToSet`).
