...
Compose wires `MIGRALOOP_PLATFORM_STORE_URL=postgres://migraloop:migraloop@platform-store:5432/migraloop` into the app and runs `migraloop run` (continuous Incremental Capture + Delivery for applied Pipelines, plus Prometheus `/metrics` on host port `9090` via `MIGRALOOP_METRICS_ADDR`). Supply Source/Target secret refs into the app environment so continuous Sync can run. Bundled Postgres ships product-owned Platform Store defaults sized for throughput evidence (`shared_buffers=512MB`, `work_mem=32MB`, `maintenance_work_mem=256MB`, `max_connections=200`, `effective_cache_size=2GB`) — still above Platform Store Guardrails floors (ADR-0010 / ADR-0031). The store data volume is also mounted read-only into the app (`MIGRALOOP_PLATFORM_STORE_DATA_DIR`) for the free-disk warn probe. Local Sync Lab Fixture uses the same Store knobs plus raised Oracle `shm_size` and MongoDB WiredTiger cache so headline Lab evidence is not routinely DB-starved. Tune volumes/resources upward as needed; do not replace the store engine or drop settings below product minimums (see [Operations](operations.md)). When Lab evidence is labeled `infra-saturated`, resize the Fixture and re-run — that outcome is not a product failure.
...
Transform Pipelines in `deployment.yaml` must use Aggregation `$…` stages (`$project`, `$match`, `$group`, …); classic step names and SQL-ish aliases are rejected—see [Rich Transform](rich-transform.md) and [Pipeline](pipeline.md).
...
migraloop lab scenario run mega-mix   # all path families + solo/mix e2e QPS + 0.7/0.95 gates (Direct ≥100k floor Lab-manual #252; Transform ≥50k #253)
