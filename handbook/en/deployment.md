...
Compose wires `MIGRALOOP_PLATFORM_STORE_URL=postgres://migraloop:migraloop@platform-store:5432/migraloop` into the app and runs `migraloop run` (Prometheus `/metrics` on host port `9090` via `MIGRALOOP_METRICS_ADDR`). Tune Postgres volumes/resources as needed; do not replace the store engine.
