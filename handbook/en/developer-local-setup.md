...
1. **Implement the engine interface**
   ...
   - Default Operator CLI `apply` / `run` / `sync` still constructs the v1 Oracle LogMiner and MongoDB adapters via those factories. For in-process seam tests, full Incremental Sync also accepts injected engines plus typed `SyncOptions` (`run_incremental_sync_with_engines`) so Fake adapters can exercise the production Sync path — including Poison / Backpressure fault knobs — without Oracle-kind string gates or process env vars. Poison quarantine, Schema Change pause, and bounded Backpressure remain distinct policies inside Incremental Sync. Legacy env knobs (`MIGRALOOP_DELIVERY_POISON_IDENTITIES`, `MIGRALOOP_SYNC_QUEUE_CAPACITY`, etc.) stay as a thin temporary compat shim for existing RQG / Lab twins via `SyncOptions::from_env_compat`.
