...
1. **Implement the engine interface**
   ...
   - Default Operator CLI `apply` / `run` / `sync` still constructs the v1 Oracle LogMiner and MongoDB adapters via those factories. Full Incremental Sync accepts typed `SyncOptions` on the runtime interface (`run_incremental_sync` / `run_incremental_sync_with_engines` / `sync_incremental_with_options`). RQG contract twins and Lab fault paths pass Poison / delay / fail-after / queue-capacity knobs as typed SyncOptions (hidden `migraloop sync` flags or explicit structs) — not process env as the primary adapter. Fake adapters can exercise the production Sync path without Oracle-kind string gates. Poison quarantine, Schema Change pause, and bounded Backpressure remain distinct policies inside Incremental Sync. Legacy fault-injection env knobs (`MIGRALOOP_DELIVERY_POISON_IDENTITIES`, `MIGRALOOP_DELIVERY_DELAY_MS`, …) remain only a thin temporary compat shim when typed overrides are unset (`SyncOptions::for_cli` / `from_env_compat`). Operator knobs such as `MIGRALOOP_SYNC_QUEUE_CAPACITY` / `MIGRALOOP_POISON_MAX_ATTEMPTS` still apply via env.
