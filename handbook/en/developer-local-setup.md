...
1. Create `lab/scenarios/<id>/` with:
   - `recipe.yaml` — recipe-driven runner interface (also catalog metadata): `id`, `summary`, **Scenario Namespace** (`source_tables`, `target_collections`, `deployment`, `pipelines`, and `lifecycle` with table column DDL + `seed_sql` and optional `mutate_sql` for the shared Namespace runner), `workload` (`concurrency`: `serial`|`parallel`, ordered prose `steps`, typed `product_path` for shared prepare→apply→mutate→sync→assert), executable `checks.correctness` (Managed present/absent, Derived/Target inspect, status text, row/document counts), optional equal-weight `thresholds` (`max_settle_ms`, `max_lag`, `max_duration_ms`, `min_rows_per_s`)
...
2. Register the Scenario id and implement thin hooks only for rare escapes for `workload.product_path` (required for catalog Scenarios). Shared Namespace wipe/prepare (and optional `mutate_sql`) come from `namespace.lifecycle`; isomorphic Managed/Derived/Target correctness comes from executable `checks.correctness` — do not copy another prepare/remove triple or fat inspect assert arms. Recipe `workload` / `namespace.lifecycle` / `checks.correctness` / `thresholds` are the recipe-driven runner interface — do not duplicate threshold values, Namespace wipe/prepare SQL, isomorphic inspect asserts, or the common product-path sequence as Rust constants.
...
1. **Implement the engine interface**
   ...
   - Discovered Source columns expose engine-agnostic `data_type` / shared `ColumnShape` at the `SourceEngine` seam. Oracle allow-list, size caps, and type-brand naming stay adapter-private (ADR-0018). NUMBER→Mongo classification lives next to `ColumnShape` in `migraloop-types` (ADR-0023) — do not keep twin helpers in capture or delivery.
   ...
   - The Deployment runtime public surface is limited to Operator Deployment verbs (apply, Incremental Sync / supervise, Pipeline lifecycle, Source Alignment Check, Drift Check, status inventory, inspect) plus those factory entry points. Do not call demoted internal helpers from outside `migraloop-runtime`. Continuous Sync / supervise prefer an open Platform Store session opened at the Operator edge.
   - Default Operator CLI `apply` / `run` / `sync` still constructs the v1 Oracle LogMiner and MongoDB adapters via those factories. Kind selection lives in the factories; apply / Incremental Sync orchestration does not re-gate on `kind == "oracle"` once an engine is factory-selected or injected. Full Incremental Sync accepts typed `SyncOptions` on the runtime interface (`run_incremental_sync` / `run_incremental_sync_with_engines`; continuous / supervise keep real poll and restart policy). Sync entry aliases that only renamed the OneShot verb were collapsed (#208). Apply / Initial Load accepts typed ApplyOptions and optional injected engines (`apply_with_options` / `apply_with_engines`). RQG contract twins and Lab fault paths pass Poison / delay / fail-after / queue-capacity knobs as typed SyncOptions (hidden `migraloop sync` flags or explicit structs), and Initial Load chunk / rate / pause / store-delay knobs as typed ApplyOptions (hidden `migraloop apply` flags or explicit structs / `apply_with_options`) — not process env as the primary adapter. Continuous Sync idle poll interval is typed on SyncOptions (`poll_interval_ms`; hidden `--sync-poll-interval-ms` on `run`). Fake adapters can exercise the production apply and Sync paths without orchestration kind gates. Poison quarantine, Schema Change pause, and bounded Backpressure remain distinct policies inside Incremental Sync. Legacy fault-injection / Lab-inject env knobs (`MIGRALOOP_DELIVERY_POISON_IDENTITIES`, `MIGRALOOP_DELIVERY_DELAY_MS`, `MIGRALOOP_INITIAL_LOAD_PAUSE_AFTER_CHUNKS`, `MIGRALOOP_INITIAL_LOAD_STORE_DELAY_MS`, …) remain only a thin temporary compat shim when typed overrides are unset (`SyncOptions::for_cli` / `ApplyOptions::for_cli` / `from_env_compat`). Operator knobs such as `MIGRALOOP_SYNC_QUEUE_CAPACITY` / `MIGRALOOP_POISON_MAX_ATTEMPTS` / `MIGRALOOP_INITIAL_LOAD_CHUNK_SIZE` / `MIGRALOOP_INITIAL_LOAD_ROWS_PER_SEC` / `MIGRALOOP_SYNC_POLL_INTERVAL_MS` still apply via env.
...
In-memory `FakeSource` / `FakeTarget` adapters exist for seam tests (including injected apply / Initial Load and full Incremental Sync); they are not a second production engine.
...
| **rqg-perf** | Fixed Direct + Transform Pipeline microbenches on contract/stub vs committed baselines (`allowed_regression_pct` ~55 for hosted-runner noise; up to 3 attempts each) | CI-parity env below, then `bash ci/rqg/run_direct_pipeline_microbench.sh` and `bash ci/rqg/run_transform_pipeline_microbench.sh` |
