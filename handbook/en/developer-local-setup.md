...
2. Register the Scenario id and implement thin hooks (Namespace prepare / Source mutate / correctness assert / remove) for `workload.product_path` (required for catalog Scenarios). Recipe `workload` / `checks` / `thresholds` are the recipe-driven runner interface — do not duplicate threshold values or the common product-path sequence as Rust constants.
