...
**Local Sync Lab** disposable defaults (`migraloop lab up`) are intentionally local-dev friendly and printed after bring-up (`ORACLE_PASSWORD=lab_oracle`, `MONGO_PASSWORD=lab_mongo`, Platform Store `migraloop`/`migraloop`). Lab Scenario runs (`migraloop lab scenario run …`) reuse those same Lab-only secret references for real `apply`/`sync` against the disposable Fixture. Use them only for Lab; never point Lab commands or Scenario configs at customer production databases.
