...
Report Platform Store health, Deployments, Pipelines, Base Datasets, Sync Health, Delivery Health, Quarantine rows, Schema Change impacts, and Derived Datasets. When Poison Change quarantine is active, Delivery Health is `unhealthy` and each quarantined Output Identity is listed as unhealthy / not aligned (ADR-0015). When blocking Schema Change pause is active, Delivery Health is `paused` and `status` lists Schema Change blocking rows (ADR-0009)—distinct from quarantine.
