...
| `MIGRALOOP_INJECT_SCHEMA_CHANGES` | 仅 Test/Lab injection：Schema Change events JSON 文件路径（`scn`、`table`、`kind`、`columns`…），用来在没有 LogMiner DDL capture 时演练 blocking DDL warn+pause（不是 production Operator 控制） |
