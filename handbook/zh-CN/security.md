...
**Local Sync Lab** 可丢弃默认（`migraloop lab up`）刻意方便本地开发，并在 bring-up 后打印（`ORACLE_PASSWORD=lab_oracle`、`MONGO_PASSWORD=lab_mongo`、Platform Store `migraloop`/`migraloop`）。Lab Scenario 运行与 Namespace 清理（`migraloop lab scenario run …`、`remove`、`--auto-remove`）会以相同的 Lab-only secret references，对可丢弃堆栈跑真实 `apply`/`sync` 与 Fixture DB 清理。仅供 Lab 使用；绝不要把 Lab 命令或 Scenario 配置指向客户生产环境数据库。
