...
| `timezone` | 可选 IANA 名称或 Oracle 风格 offset（`+09:00` / `±HH:MM`）。`apply` 时接受。在 naive DATE/TIMESTAMP 需要解读且 Source DB timezone 不可读时使用 |
