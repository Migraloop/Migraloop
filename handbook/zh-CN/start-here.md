...
若要可丢弃的 **Local Sync Lab** Fixture（Oracle + MongoDB + Platform Store + app，无默认 Deployment/Pipelines）：`migraloop lab up` / `status` / `down`。可选的 **Lab Scenarios**（catalog 来自 `lab/scenarios/<id>/recipe.yaml`；例如 `migraloop lab scenario list` / `run direct-pipeline` / `run rt-project` / `run rt-filter` / `run transform-pipeline` / `run concurrent-source-workload` / `run bulk-load`）会在 Scenario Namespace 内走真实 apply/sync；重跑会先 wipe Namespace，另可用 `scenario remove` / `--auto-remove` 清理。手动验证（ADR-0025）—不是 Release Quality Gate。见 [Deployment](deployment.md)、[CLI 与 Config 参考](cli-and-config.md)，以及 [Developer local setup](developer-local-setup.md)（feature-time 编写路径）。
