...
若要可丢弃的 **Local Sync Lab** Fixture（Oracle + MongoDB + Platform Store + app，无默认 Deployment/Pipelines）：`migraloop lab up` / `status` / `down`。可选的 **Lab Scenarios**（例如 `migraloop lab scenario list` / `run direct-pipeline`）会在 Scenario Namespace 内走真实 apply/sync — 见 [Deployment](deployment.md) 与 [CLI 与 Config 参考](cli-and-config.md)。
