...
- **Developer** 若要设置 monorepo，请直接看 [Developer 本地设置](developer-local-setup.md)。新增 Source 或 Target engine 请用该页 checklist（interface + prerequisites／文档 + Lab Scenario + CI contract twin）——不要重写 Sync／Rich Transform／Delivery／runtime。
...
若要可丢弃的 **Local Sync Lab** Fixture（Oracle + MongoDB + Platform Store + app，无默认 Deployment/Pipelines）：`migraloop lab up` / `status` / `down`。`lab status` 会回报 Fixture 就绪状态，以及哪个 Scenario Namespace 为 active 或 leftover（或 `(none)`）。可选的 **Lab Scenarios**（catalog 来自 `lab/scenarios/<id>/recipe.yaml`；运行走 recipe-driven runner 真实 product path—recipe `product_path` 步骤共用 Namespace lifecycle + prepare／apply／sync；`namespace.lifecycle` 负责 wipe／seed（可选 mutate SQL）；thin hooks 负责 rare escapes；`checks.correctness` 为可执行 inspect vocabulary；poison／delay／fail-after／queue-capacity 等 Lab escapes 使用 typed SyncOptions CLI flags，Initial Load throttle／pause／store-delay knobs 使用 typed ApplyOptions CLI flags（不以 process env 作为主要 adapter）；例如 `migraloop lab scenario list` / `run direct-pipeline` / `run rt-project` / `run rt-filter` / `run rt-field-ops` / `run rt-equilookup` / `run rt-union` / `run rt-unwind` / `run rt-distinct-addtoset` / `run transform-pipeline`（groupBy sum/count/min/max/avg） / `run concurrent-source-workload` / `run change-ordering` / `run bulk-load` / `run mega-mix`（Direct ≥100k／Transform ≥50k floors 为 Lab-manual #252/#253） / `run idempotent-redelivery` / `run pause-resume` / `run remove-pipeline` / `run change-pipeline` / `run poison-quarantine` / `run schema-change-pause` / `run source-alignment` / `run drift-check` / `run bounded-backpressure` / `run observability-surface` / `run platform-store-guardrails` / `run backward-compatible-upgrades` / `run initial-load-throttled`）会在 Scenario Namespace 内走真实 apply/sync（Transform Scenarios 以建议的 Aggregation／SQL-like DX 编写—见 [Rich Transform](rich-transform.md)；classic steps 仍 Upgrade Compatible；执行期间 Lab 会暂停 Fixture `app` 以独占 host Sync，结束后再恢复）；Scenario `run` 会在 apply/sync 前拒绝非 Lab／生产环境的 Source/Target engine 绑定。重跑会先 wipe Namespace，另可用 `scenario remove` / `--auto-remove` 清理。若要在 Scenario recipes 之外做 DB-level restore/load，请用 `lab/escape-hatch/` 搭配 Lab 连接细节，再接普通 `apply`／`status`／inspect—同样不是 Release Quality Gate。手动验证（ADR-0025）。嵌套 Docker／**Cursor Cloud** storage-driver 说明（`fuse-overlayfs` 或 `vfs`）：见 [Developer local setup](developer-local-setup.md) 与 [Deployment](deployment.md)。另见 [CLI 与 Config 参考](cli-and-config.md)。
