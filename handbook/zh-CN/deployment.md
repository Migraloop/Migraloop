...
Compose 会把 `MIGRALOOP_PLATFORM_STORE_URL=postgres://migraloop:migraloop@platform-store:5432/migraloop` 注入 app，并执行 `migraloop run`（经 `MIGRALOOP_METRICS_ADDR` 在 host port `9090` 提供 Prometheus `/metrics`）。可调整 Postgres volumes/resources，但不要更换 store 引擎。
