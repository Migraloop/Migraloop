...
Compose 定义：`lab/compose.yaml`（project `migraloop-lab`）。Lab Oracle init 会启用 ARCHIVELOG 与 database supplemental logging 以供 LogMiner；**不会**预先套用任何 Deployment 或 Pipelines—那些来自 Lab Scenario 或你自己的 `migraloop apply`。与上方默认双 container 安装、以及 CI 使用的 contract/stub harness 都不同。

资源提醒：Lab Oracle（Free）通常需要数 GB RAM，第一次拉 image／开机可能要数分钟。
