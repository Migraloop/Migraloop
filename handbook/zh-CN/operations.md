...
- Deployment config `apiVersion` 在 major `1` 上为 SemVer-compatible：正式写法 `migraloop.dev/v1`，较旧可接受形式如 `migraloop.dev/v1.0` / `migraloop.dev/v1.0.0` 仍可套用；不相容 major 会以清楚错误拒绝
