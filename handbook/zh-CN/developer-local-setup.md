...
1. **实现 engine interface**
   ...
   - 默认 Operator CLI `apply`／`run`／`sync` 仍经由上述 factories 构建 v1 Oracle LogMiner 与 MongoDB adapters。完整 Incremental Sync 在 runtime interface 接受 typed `SyncOptions`（`run_incremental_sync`／`run_incremental_sync_with_engines`／`sync_incremental_with_options`）。RQG contract twins 与 Lab fault paths 以 typed SyncOptions（隐藏的 `migraloop sync` flags 或显式 structs）传入 Poison／delay／fail-after／queue-capacity knobs——不以 process env 作为主要 adapter。Fake adapters 可走 production Sync path 且不依赖 Oracle-kind string gates。Poison quarantine、Schema Change pause、与 bounded Backpressure 在 Incremental Sync 内仍是 distinct policies。既有 fault-injection env knobs（`MIGRALOOP_DELIVERY_POISON_IDENTITIES`、`MIGRALOOP_DELIVERY_DELAY_MS` 等）仅在未设置 typed overrides 时保留为薄的临时 compat shim（`SyncOptions::for_cli`／`from_env_compat`）。Operator knobs 如 `MIGRALOOP_SYNC_QUEUE_CAPACITY`／`MIGRALOOP_POISON_MAX_ATTEMPTS` 仍可经由 env 生效。
