...
2. 注册 Scenario id，并为 `workload.product_path`（catalog Scenarios 必填）只实现 rare escapes + correctness assert 的 thin hooks。共用 Namespace wipe／prepare（以及可选 `mutate_sql`）来自 `namespace.lifecycle`——不要再复制另一份 prepare／remove 三元组。Recipe 的 `workload`／`namespace.lifecycle`／`checks`／`thresholds` 是 recipe-driven runner 接口——不要把 threshold 数值、Namespace wipe／prepare SQL，或共用 product-path 序列再复制成 Rust constants。
...
Seam 测试可用 in-memory `FakeSource`／`FakeTarget`（含 injected 完整 Incremental Sync）；它们不是第二个 production engine。
