...
2. 注册 Scenario id；若使用 `workload.product_path`，实现 thin hooks（Namespace prepare／Source mutate／correctness assert／remove）；仅在尚无法共用该 path 时才写完整 adapter。Recipe 的 `workload`／`checks`／`thresholds` 是 recipe-driven runner 接口——不要把 threshold 数值或共用 product-path 序列再复制成 Rust constants。
...
1. **实现 engine interface**
   ...
   - 默认 Operator CLI `apply`／`run`／`sync` 仍经由上述 factories 构建 v1 Oracle LogMiner 与 MongoDB adapters。in-process seam 测试可用 injected engines 加上 typed `SyncOptions`（`run_incremental_sync_with_engines`）跑完整 Incremental Sync，让 Fake adapters 走 production Sync path——含 Poison／Backpressure fault knobs——且不依赖 Oracle-kind string gates 或 process env vars。Poison quarantine、Schema Change pause、与 bounded Backpressure 在 Incremental Sync 内仍是 distinct policies。既有 env knobs（`MIGRALOOP_DELIVERY_POISON_IDENTITIES`、`MIGRALOOP_SYNC_QUEUE_CAPACITY` 等）通过 `SyncOptions::from_env_compat` 保留为薄的临时 compat shim，让既有 RQG／Lab twins 继续通过。
