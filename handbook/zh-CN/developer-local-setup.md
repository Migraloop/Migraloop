...
2. 注册 Scenario id，并为 `workload.product_path`（catalog Scenarios 必填）实现 thin hooks（Namespace prepare／Source mutate／correctness assert／remove）。Recipe 的 `workload`／`checks`／`thresholds` 是 recipe-driven runner 接口——不要把 threshold 数值或共用 product-path 序列再复制成 Rust constants。
