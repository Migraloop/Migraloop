...
This project does **not** pursue a refactor that moves remaining human-readable apply / Incremental Sync progress, ALERT, quarantine, and backpressure companion lines out of the Deployment runtime into the CLI adapter as the sole formatter.
...
Forcing the leftover println → CLI move would invent a continuous-run event sink, rewrite RQG / Lab scrapers that assert Operator wording, and churn three handbook locales without changing Operator-visible outcomes. Parent #199 explicitly allowed deferring this last optional slice when stronger candidates removed the pain.
