...
| Rich Transform `distinct` / `addToSet` (Maintenance State) | `rt-distinct-addtoset` | `cli_distinct_addtoset_affect.rs` | Happy path `$group` / `$group`+`$addToSet`; value-level Affect skip + Maintenance State; classic forms still parse |
