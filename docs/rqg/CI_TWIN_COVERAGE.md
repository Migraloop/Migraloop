...
| Multi-table Transform Pipeline (`groupBy` sum/count/min/max/avg) | `transform-pipeline` | `cli_groupby_sum_affect.rs`, `cli_groupby_rich_aggs_affect.rs`, `cli_multi_table_incremental.rs` | Lab + twins author `$group` only (#250 / ADR-0030); Affect Analysis + multi-table Direct/Transform settle |
| Rich Transform `project` | `rt-project` | `cli_transform_pipeline.rs` (Aggregation `$project`/`$match` happy path + classic/SQL-ish reject) | #250 Aggregation-only authoring; unit IR/Affect in `migraloop-transform` |
| Rich Transform `filter` | `rt-filter` | `cli_transform_pipeline.rs` (Aggregation `$match` happy path + classic/SQL-ish reject) | |
| Rich Transform `addFields` / `rename` / `remove` | `rt-field-ops` | `cli_transform_field_ops.rs` | Lab + twin happy/Affect paths use `$addFields`/`$unset`/`$rename` only |
...
| Rich Transform `unwind` | `rt-unwind` | `cli_unwind_affect.rs` | Happy path `$lookup`+`$unwind` flatten → Derived/Delivery; Affect Analysis insert/update/delete; unsupported options/script reject |
| Rich Transform `distinct` / `addToSet` (Maintenance State) | `rt-distinct-addtoset` | `cli_distinct_addtoset_affect.rs` | Happy path `$group` / `$group`+`$addToSet` only; value-level Affect skip + Maintenance State; classic forms rejected |
| Rich Transform `groupBy` / `sum` (also under contention) | `transform-pipeline`, `concurrent-source-workload` | `cli_groupby_sum_affect.rs`, `cli_multi_table_incremental.rs` | CI: `$group`/`$sum` multi-table settle after both Bases change. OS-parallel Source sessions stay Lab-manual |
| Rich Transform `groupBy` `count` / `min` / `max` / `avg` | `transform-pipeline` | `cli_groupby_rich_aggs_affect.rs` | `$group` accumulators; Affect Analysis unused-field skip + incremental Derived |
| Change Ordering / confluence (same-key order, cross-key interleave, min Base recompute) | `change-ordering` | `cli_change_ordering.rs` | Same-key A→B→C final Managed; cross-key interleave finals; min extreme delete → Base recompute. No Alignment/Drift on the normal path. Live Oracle capture order stays Lab-manual |
...
| Observability Surface (logs, health, Prometheus) | `observability-surface` | `cli_observability_surface.rs` | Structured JSON operator events on sync; `status` Sync/Delivery Health + Pipeline; `run --metrics-addr` Prometheus `/metrics` exposes lag + alertable failure counters + component pressure (`migraloop_component_pressure` / `migraloop_component_saturated` for app/source/platform_store/target) |
| Capacity Estimate (limiting component + coarse max e2e QPS) | _(Lab Scenario reports share pressure / `infra-saturated` labels; live verb is Operator CLI)_ | `cli_capacity_estimate.rs` | `capacity-estimate` output shape (`limiting_component`, `max_e2e_qps`, `infra_saturated`); injectable pressure selects limiting component; read-only (no Deployment mutation); `status` lists same stable component pressure names. Lab `INFRA-SATURATED` outcome probe in `cli_lab_scenario.rs` |
...
| Chunked / rate-limited / pausable Initial Load with backoff | `initial-load-throttled` | `cli_initial_load_chunked.rs` (also in-process: `deployment_runtime_apply_options.rs`) | Contract/stub: typed ApplyOptions (`--initial-load-chunk-size` / `--initial-load-rows-per-sec` / `--initial-load-pause-after-chunks` / `--initial-load-store-delay-ms`) drive bounded progress, observable rate_limit, pause/resume with durable watermark, and store-pressure backoff. Legacy `MIGRALOOP_INITIAL_LOAD_*` env remains a thin compat shim only. Live Oracle volume + real pause timing stay Lab-manual |
| Mega-mix (all path families + solo/mix e2e QPS + 0.7/0.95 gates) | `mega-mix` | path-family twins above (`cli_direct_pipeline_*`, `cli_transform_*`, `cli_equilookup_affect.rs`, `cli_union_affect.rs`, `cli_unwind_affect.rs`, `cli_groupby_sum_affect.rs`, `cli_pause_resume_pipeline.rs`, `cli_capacity_estimate.rs`); catalog list + gate math in `cli_lab_scenario.rs` / `lab_scenario::mega_mix` unit tests | CI covers shipped path-family correctness + Capacity Estimate / pressure shape. Solo/mix e2e QPS, Direct/Transform path aggregates, 0.7/0.95 gate results, and 100k/50k floors stay **Lab-manual** on `lab scenario run mega-mix` (ADR-0025 / ADR-0028 / ADR-0031)—never RQG accept evidence |
