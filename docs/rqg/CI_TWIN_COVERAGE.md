...
| Rich Transform `union` (multi-Base Transform Pipeline) | `rt-union` | `cli_union_affect.rs` | Multi-Base Initial Load + Derived concatenation; Affect Analysis on both contributing Bases; `$unionWith`/script reject |
| Rich Transform `unwind` | `rt-unwind` | `cli_unwind_affect.rs` | equiLookup+unwind flatten → Derived/Delivery by unwound Output Identity; Affect Analysis insert/update/delete on primary and foreign Bases; `$unwind`/unsupported forms/script reject |
