...
| Chunked / rate-limited / pausable Initial Load with backoff | `initial-load-throttled` | `cli_initial_load_chunked.rs` | Contract/stub: bounded `MIGRALOOP_INITIAL_LOAD_CHUNK_SIZE` progress events; observable `MIGRALOOP_INITIAL_LOAD_ROWS_PER_SEC`; pause/resume via `MIGRALOOP_INITIAL_LOAD_PAUSE_AFTER_CHUNKS` with durable watermark; backoff under `MIGRALOOP_INITIAL_LOAD_STORE_DELAY_MS`. Live Oracle volume + real pause timing stay Lab-manual |
