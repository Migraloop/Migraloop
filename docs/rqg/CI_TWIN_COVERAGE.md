...
| Rich Transform `project` | `rt-project` | `cli_transform_pipeline.rs` (`project` paths + Aggregation `$project`/`$match` DX) | Classic form + #232 Aggregation DX expand (unit IR/Affect equivalence in `migraloop-transform`) |
| Rich Transform `filter` | `rt-filter` | `cli_transform_pipeline.rs` (`filter` paths + Aggregation `$match` DX) | |
| Rich Transform `addFields` / `rename` / `remove` | `rt-field-ops` | `cli_transform_field_ops.rs` | Affect Analysis unused-field skip after `remove`; Aggregation `$addFields`/`$unset`/`$rename` covered by transform crate equivalence tests |
| Rich Transform `equiLookup` (multi-Base Transform Pipeline) | `rt-equilookup` | `cli_equilookup_affect.rs` | Multi-Base Initial Load + Derived join array; Affect Analysis on primary and foreign Bases; constrained `$lookup` accepted (#232); `pipeline`/`let`/script reject |
| Rich Transform `union` (multi-Base Transform Pipeline) | `rt-union` | `cli_union_affect.rs` | Multi-Base Initial Load + Derived concatenation; Affect Analysis on both contributing Bases; constrained `$unionWith` accepted (#232); nested `pipeline`/script reject |
| Rich Transform `unwind` | `rt-unwind` | `cli_unwind_affect.rs` | equiLookup+unwind flatten → Derived/Delivery by unwound Output Identity; Affect Analysis insert/update/delete on primary and foreign Bases; constrained `$unwind` accepted (#232); unsupported options/script reject |
| Rich Transform `distinct` / `addToSet` (Maintenance State) | `rt-distinct-addtoset` | `cli_distinct_addtoset_affect.rs` | Value-level Affect Analysis skip for duplicate distinct keys / addToSet members; Maintenance State persisted only for these ops; incremental Derived + Delivery |
